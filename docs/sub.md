...
Apple-клиент умеет импортировать подписку из HTTP/HTTPS URL или из вставленного текста `sub.md`.
Импорт отдельного `olcrtc://` URI создаёт самостоятельный профиль, а не меняет выбранный профиль.

Подписки, импортированные из HTTP/HTTPS URL, сохраняют `sourceURL`. По кнопке обновления рядом с подпиской Apple-клиент заново скачивает этот URL и синхронизирует список узлов:

- существующие узлы обновляются свежими данными из подписки;
- узлы, которых больше нет в новом `sub.md`, удаляются;
- новые узлы добавляются;
- локальные runtime-настройки совпавших узлов сохраняются: SOCKS port, SOCKS credentials, DNS, debug logging и start timeout.

Для стабильного сопоставления клиент хранит исходную строку `olcrtc://` каждого узла как локальные метаданные профиля. Для старых сохранённых подписок, где этих метаданных ещё нет, используется fallback по carrier, transport, room ID, client ID и key.

Поле `#refresh:` сейчас сохраняется как метаданные подписки. Автоматическое фоновое обновление клиент пока не выполняет; обновление запускается пользователем вручную.
