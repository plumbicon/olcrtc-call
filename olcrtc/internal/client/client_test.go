...
func TestSocks5HandshakeCredentialsStillAllowNoAuth(t *testing.T) {
	c := &Client{socksUser: "user", socksPass: "pass"}
	server, client := net.Pipe()
	defer func() {
		_ = server.Close()
	...
	if _, err := client.Write([]byte{5, 1, socksMethodNoAuth}); err != nil {
		t.Fatalf("Write() error = %v", err)
	}
	resp := make([]byte, 2)
	if _, err := io.ReadFull(client, resp); err != nil {
	...
	if !bytes.Equal(resp, []byte{5, socksMethodNoAuth}) {
		t.Fatalf("handshake response = %v, want [5 %d]", resp, socksMethodNoAuth)
	}
}
...
func TestSocks5HandshakeRejectsMissingAuthMethod(t *testing.T) {
	tests := []struct {
		name    string
		client  *Client
		methods []byte
		...
		{
			name:    "no auth required but not offered",
			client:  &Client{},
			methods: []byte{socksMethodUserPass},
		},
		{
			name:    "credentials configured but neither compatible method offered",
			client:  &Client{socksUser: "user", socksPass: "pass"},
			methods: []byte{0x01},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			go func() {
				done <- tt.client.socks5Handshake(server)
			}()

			greeting := append([]byte{5, byte(len(tt.methods))}, tt.methods...)
			...
			if !bytes.Equal(resp, []byte{5, socksMethodNotAvailable}) {
				t.Fatalf("handshake response = %v, want [5 %d]", resp, socksMethodNotAvailable)
			}

			if err := <-done; !errors.Is(err, ErrProxyAuth) {
				t.Fatalf("socks5Handshake() error = %v, want ErrProxyAuth", err)
			}
		})
	}
}
...
func TestSocks5RequestIPv6(t *testing.T) {
	...
	if res.addr != "2001:db8::1" || res.port != 443 {
		t.Fatalf("socks5Request() = (%q, %d), want (2001:db8::1, 443)", res.addr, res.port)
	}
}
