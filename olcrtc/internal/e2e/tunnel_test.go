...
func startEchoServer(t *testing.T) string {
	...
	return startEchoServerOn(t, "tcp4", "127.0.0.1:0")
}

func startEchoServerIPv6(t *testing.T) string {
	t.Helper()

	return startEchoServerOn(t, "tcp6", "[::1]:0")
}

func startEchoServerOn(t *testing.T, network, address string) string {
	t.Helper()
...
func connectViaSOCKS(t *testing.T, socksAddr, targetAddr string) net.Conn {
	...
	return connectViaSOCKSWithAddressType(t, socksAddr, targetAddr, 1)
}

func connectViaSOCKSIPv6(t *testing.T, socksAddr, targetAddr string) net.Conn {
	t.Helper()

	return connectViaSOCKSWithAddressType(t, socksAddr, targetAddr, 4)
}

func connectViaSOCKSWithAddressType(t *testing.T, socksAddr, targetAddr string, addrType byte) net.Conn {
	t.Helper()
	...
	addrBytes := socksRequestAddressBytes(t, host, addrType)
...
func socksRequestAddressBytes(t *testing.T, host string, addrType byte) []byte {
	t.Helper()
	...
	case 3:
		if len(host) > 255 {
			t.Fatalf("target domain %q is too long for SOCKS5", host)
		}
		return []byte(host)
	case 4:
		ip := net.ParseIP(host).To16()
		if ip == nil || ip.To4() != nil {
			t.Fatalf("target host %q is not IPv6", host)
		}
		return ip
	default:
		t.Fatalf("unsupported test address type %d", addrType)
		return nil
	}
}
...
func TestClientServerSOCKSTunnelOverMemoryDatachannelIPv6(t *testing.T) {
	echoAddr := startEchoServerIPv6(t)
	rt := startTunnel(t, "client-1", "client-1")
	defer rt.stop(t)

	conn := connectViaSOCKSIPv6(t, rt.socksAddr, echoAddr)
	defer func() { _ = conn.Close() }()

	payload := []byte("olcrtc-e2e-ipv6-payload\n")
	if _, err := conn.Write(payload); err != nil {
		t.Fatalf("write tunneled ipv6 payload: %v", err)
	}
	if err := conn.SetReadDeadline(time.Now().Add(3 * time.Second)); err != nil {
		t.Fatalf("set ipv6 read deadline: %v", err)
	}
	line, err := bufio.NewReader(conn).ReadBytes('\n')
	if err != nil {
		t.Fatalf("read tunneled ipv6 echo: %v", err)
	}
	if !bytes.Equal(line, payload) {
		t.Fatalf("ipv6 echo = %q, want %q", line, payload)
	}
}
